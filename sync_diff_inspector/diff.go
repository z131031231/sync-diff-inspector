...
type Diff struct {
	sourceDBs         map[string]DBConfig   //源库
	targetDB          DBConfig              //目标库
...
func NewDiff(ctx context.Context, cfg *Config) (diff *Diff, err error) {
	diff = &Diff{
		sourceDBs:         make(map[string]DBConfig), //源库
		chunkSize:         cfg.ChunkSize,             //chunk大小
		sample:            cfg.Sample,
		checkThreadCount:  cfg.CheckThreadCount,     //检查chunk的线程数
		useChecksum:       cfg.UseChecksum,
		useCheckpoint:     cfg.UseCheckpoint,
		onlyUseChecksum:   cfg.OnlyUseChecksum,
		ignoreDataCheck:   cfg.IgnoreDataCheck,     //忽略数据校验
		ignoreStructCheck: cfg.IgnoreStructCheck,   //忽略表结构校验
